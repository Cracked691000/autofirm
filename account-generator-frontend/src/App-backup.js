...
export default function AccountGeneratorApp() {
  ...
  const startGeneration = async () => {
    ...
    if (useAiSolver && !geminiApiKey.trim()) {
      alert('Please enter a Gemini API key or disable AI solver');
      return;
    }
    ...
    try {
      const response = await fetch(`${backendUrl}/api/generate`, {
        ...
        body: JSON.stringify({
          ...
          useAi: useAiSolver,
          geminiApiKey: useAiSolver ? geminiApiKey : null
        })
      });
      ...
      <div style={{ maxWidth: '600px', margin: '0 auto' }}>
        ...
        <div style={{ backgroundColor: '#ffffff', borderRadius: '16px', boxShadow: '0 10px 25px rgba(0,0,0,0.05)', overflow: 'hidden' }}>
          ...
          <div style={{ padding: '24px' }}>
            ...
            {/* AI Solver Toggle */}
            <div style={{ marginBottom: '25px', padding: '16px', backgroundColor: '#f9fafb', borderRadius: '12px', border: '1px solid #e5e7eb' }}>
              <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '12px' }}>
                ...
                <label style={{ position: 'relative', display: 'inline-block', width: '44px', height: '24px', cursor: isRunning ? 'not-allowed' : 'pointer' }}>
                  <input 
                    type="checkbox" 
                    checked={useAiSolver}
                    onChange={(e) => setUseAiSolver(e.target.checked)}
                    disabled={isRunning}
                  ...
                  <span style={{
                    ...
                    right: 0,
                    bottom: 0,
                    backgroundColor: useAiSolver ? '#3b82f6' : '#cbd5e1',
                    transition: '0.3s',
                    borderRadius: '24px'
                  }}>
                    <span style={{
              ...
              {useAiSolver && (
                <div style={{ animation: 'fadeIn 0.3s ease-out' }}>
                  ...
                  />
                  <p style={{ fontSize: '11px', color: '#6b7280', marginTop: '6px', marginBottom: 0, lineHeight: '1.4' }}>
                    Get your free API key at <a href="https://aistudio.google.com/apikey" target="_blank" rel="noopener noreferrer" style={{ color: '#3b82f6', textDecoration: 'none', fontWeight: '600' }}>Google AI Studio</a>
                  </p>
                </div>
              )}
            </div>
            ...
            <button 
              ...
              style={{ 
                ...
                color: '#ffffff', fontWeight: '600', fontSize: '16px', cursor: isRunning || !connected ? 'not-allowed' : 'pointer',
                display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '8px',
                boxShadow: '0 4px 6px -1px rgba(0, 0, 0, 0.1)', opacity: !connected ? 0.7 : 1
              }}
            >
          ...
          <div style={{ 
            ...
            <div style={{ 
              ...
              <div style={{ position: 'relative', borderRadius: '8px', overflow: 'hidden', border: '2px solid #e5e7eb', marginBottom: '12px' }}>
                ...
                {/* Clickable Grid Overlay - Aligned to actual captcha images */}
                <div style={{ 
                  position: 'absolute', 
                  top: '25%', 
                  left: '0', 
